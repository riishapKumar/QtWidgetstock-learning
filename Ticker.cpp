...
static const std::vector<QString> defaultTickers = { "AMD.US","AAPL.US","GOOG.US","CL.F","GC.F","EURUSD","QQQ.US"};
...
std::vector<QString> Ticker::loadTickerSymbolsFromSettingsFile() {
    ...
    if (savedTickers.fileIsValid()) {
        tempTickerSymbols = savedTickers.loadContents(); // code is sometrhing else file dir is broken rn.
    }
    else {
